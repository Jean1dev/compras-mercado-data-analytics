...
The system SHALL route all receipt-extraction LLM calls through a LiteLLM proxy using an OpenAI-compatible API. The system MUST require `LITELLM_BASE_URL`, `LITELLM_API_KEY`, and `LLM_MODEL` environment variables at startup. The system MUST NOT require or use `ANTHROPIC_API_KEY`.

#### Scenario: Startup with valid LiteLLM configuration

- **WHEN** the application starts with `LITELLM_BASE_URL`, `LITELLM_API_KEY`, and `LLM_MODEL` defined
